...
//SubPriority        :从优先级
void dev_DMA_NVIC_Configuration(IRQn_Type DMAx_Channelx_IRQn,u8 PreemptionPriority,u8 SubPriority)
{
...
void* dev_spi_init(struct dev_spi * spi, enum en_spi_port port, enum en_spi_mode mode, uint16_t spi_baudrate_prescaller)
{
	...
	spi->dma_struct.DMA_PeripheralBaseAddr = (uint32_t) &spi->spi->DR;    //DMA外设基地址
	spi->dma_struct.DMA_PeripheralInc      = DMA_PeripheralInc_Disable;   //外设地址寄存器不变
	spi->dma_struct.DMA_MemoryInc          = DMA_MemoryInc_Enable;        //内存地址寄存器递增
	spi->dma_struct.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Byte; /* 8位*/
	spi->dma_struct.DMA_MemoryDataSize     = DMA_MemoryDataSize_Byte;     //8位的数据
	spi->dma_struct.DMA_Mode               = DMA_Mode_Normal;             //工作在正常模式
	spi->dma_struct.DMA_Priority           = DMA_Priority_High;           //DMA通道 x拥有中优先级 
	spi->dma_struct.DMA_M2M                = DMA_M2M_Disable;
	
    DMA_Init(spi->dma_rx_ch, &spi->dma_struct);
	
    NVIC_EnableIRQ((IRQn_Type)spi->dma_rx_iqrn);
    NVIC_EnableIRQ((IRQn_Type)spi->dma_tx_iqrn);
	
    DMA_ITConfig(spi->dma_rx_ch, DMA_IT_TC, ENABLE);
    DMA_ITConfig(spi->dma_tx_ch, DMA_IT_TC, ENABLE);
	
	DMA_Cmd(spi->dma_rx_ch, DISABLE);
	DMA_Cmd(spi->dma_tx_ch, DISABLE);
	
    dev_DMA_NVIC_Configuration((IRQn_Type)spi->dma_rx_iqrn,3,0);
	dev_DMA_NVIC_Configuration((IRQn_Type)spi->dma_tx_iqrn,3,0);
...
inline void dev_spi_send8(struct dev_spi * spi, uint8_t * data, size_t data_len)
{
    ...
    spi->dma_struct.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Byte;
 	...
 	DMA_SetCurrDataCounter(spi->dma_rx_ch,data_len);  //DMA通道的DMA缓存的大小
	dev_spi_tx(spi);
}
...
////SPI主机发送一个数据
//u16 dev_SPI_SendByte(uint16_t dat)
//{

//    dev_spi_flag = 1;
//    TM_DMA_SetCountAndState(DMA1_Channel3,1,ENABLE); //DMA发送使能
//    while(dev_spi_flag);                             //等待完成
//    dev_spi_flag = 1;
